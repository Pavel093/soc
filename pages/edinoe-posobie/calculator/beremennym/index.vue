<template>
  <Header />
  <article class="page-container">
    <header class="page-intro">
      <h1>Калькулятор единого пособия для беременных 2025</h1>
      <p class="description">
        Рассчитайте точный размер выплаты онлайн за 2 минуты. Калькулятор автоматически определит ваш регион и подставит актуальный прожиточный минимум для максимально точного расчета.
      </p>
    </header>

    <section class="calculator-block">
      <h2>Начните расчет прямо сейчас</h2>
      <SmartCalculator />
    </section>

    <section class="features-section">
      <h2>Почему наш калькулятор — это удобно?</h2>
      <div class="features-grid">
        <div class="feature-item">
          <div class="icon">✅</div>
          <h3>Актуально на 2025 год</h3>
          <p>Учитываем все последние изменения законодательства и актуальные прожиточные минимумы по регионам.</p>
        </div>
        <div class="feature-item">
          <div class="icon">🔒</div>
          <h3>Анонимно и бесплатно</h3>
          <p>Не нужно вводить телефон, почту или личные данные. Все расчеты конфиденциальны.</p>
        </div>
        <div class="feature-item">
          <div class="icon">🎯</div>
          <h3>Точный результат</h3>
          <p>Задаем правильные вопросы о доходах, имуществе и составе семьи, чтобы вы получили достоверный ответ.</p>
        </div>
      </div>
    </section>

    <section class="faq-section">
      <h2>Частые вопросы о пособии для беременных</h2>
      <div v-for="(faq, index) in faqs" :key="index" class="faq-item">
        <details>
          <summary>{{ faq.question }}</summary>
          <p>{{ faq.answer }}</p>
        </details>
      </div>
    </section>
  </article>
  <Footer />
</template>

<script setup>
useSeoMeta({
  title: 'Калькулятор единого пособия для беременных 2025 — Рассчитать онлайн',
  description: 'Рассчитайте онлайн размер единого пособия для беременных женщин. Калькулятор автоматически определит ваш регион и учтет актуальный прожиточный минимум на 2025 год.',
  keywords: 'единое пособие, пособие беременным, калькулятор пособий, онлайн расчет, 2025, прожиточный минимум'
})

const faqs = [
  {
    question: 'На каком сроке можно подать заявление?',
    answer: 'Заявление на единое пособие можно подать после наступления 12 недель беременности. Однако выплата будет назначена за период начиная с 6 недель беременности, но не ранее постановки на учет в медицинской организации.'
  },
  {
    question: 'Какой доход семьи учитывается при назначении пособия?',
    answer: 'Учитываются доходы всех членов семьи за 12 календарных месяцев, предшествующих 1 месяцу перед месяцем подачи заявления. Например, если вы подаете заявление в июле 2025 года, будут учитываться доходы с июня 2024 по май 2025 года.'
  },
  {
    question: 'Какую сумму я буду получать?',
    answer: 'Размер пособия зависит от нуждаемости семьи и может составлять 50%, 75% или 100% от прожиточного минимума для трудоспособного населения, установленного в вашем регионе.'
  }
]
</script>

<style scoped lang="scss">
@import '~/assets/scss/calculator-landing-page.scss';
</style>